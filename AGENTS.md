...
- **Focus on the task**: Don't be an overachiever. Aim for perfection in execution rather than adding extras.
- **Use MCP tools**:
  - `deepwiki` MCP for project specific documentation and implementation details
- **Maintain efficiency files**:
  - `.deepwiki`: Repository for relevant DeepWiki repositories
  - `.gitignore`: Keep accurate to avoid repository bloat

## Requirements Management

- **RFC2119**: Only use **MUST** keywords to define unambiguous requirements
- **Gherkin**: Only specify acceptance criteria according to the Given-When-Then formula
...
#### Efficiency Maintenance Guidelines
